...
async def on_message(message):
    ...
    # 권영훈 대답 리스트
    main_message = ['http://cdn.discordapp.com/attachments/1043962135744610445/1045306360448958534/image0.jpg',
...
async def play(ctx, *, url):
    ...
    if ctx.message.channel.id == 1045278508580098088:
        ...
        if not bot.user.is_playing():
            with YoutubeDL(YDL_OPTIONS) as ydl:
                info = ydl.extract_info(url, download=False)
            URL = info['formats'][0]['url']
            vc.play(FFmpegPCMAudio(URL, **FFMPEG_OPTIONS))
            embed = discord.Embed(title="음악 재생중", description=f"{url}", color=0x00ff00)
            sliced_url = url[32:]
